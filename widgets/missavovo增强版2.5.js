...
async function buildPeopleContextWithAvatar(id, title, avatar = "") {
    if (!id) return null;
    const resolvedAvatar = avatar || await resolvePeopleAvatar(id);
    return buildPeopleContext(id, title, resolvedAvatar);
}

function mergePeopleItems(target, people) {
    if (!people) return;
    const existingIndex = target.findIndex((item) => item.id === people.id || normalizePeopleKey(item.title) === normalizePeopleKey(people.title));
    if (existingIndex >= 0) {
        if (!target[existingIndex].avatar && people.avatar) target[existingIndex].avatar = people.avatar;
        return;
    }
    target.push(people);
}

function extractPeopleFromDetailHtml(html) {
    ...
    $('meta[property="og:video:actor"], meta[property="video:actor"], meta[name="video:actor"]').each((_, el) => {
        ...
        content.split(",").map((name) => normalizeDisplayTitle(name)).filter(Boolean).forEach((name) => metaActressNames.push(name));
    });

    $(".text-secondary").each((_, el) => {
        const $row = $(el);
        const label = normalizeDetailLabel($row.find("span").first().text());
        if (!isActressLabel(label)) return;
        $row.find('a[href*="/actresses/"], a[href*="actresses"]').each((_, actressEl) => {
            const $actress = $(actressEl);
            const href = $actress.attr("href") || "";
            const rawName = normalizeDisplayTitle($actress.text());
            ...
            const people = buildPeopleItem(matchedMetaName || rawName, getImageFromElement($, $actress), href);
            mergePeopleItems(peoples, people);
        });
    });
...
async function enrichListPeopleFromDetails(items) {
    const linkItems = items.filter((item) => item && item.type === "link" && item.link);
    await Promise.all(linkItems.map(async (item) => {
        try {
            const res = await Widget.http.get(item.link, { headers: HEADERS });
            const detailPeoples = extractPeopleFromDetailHtml(res.data);
            if (!detailPeoples.length) return;
            const peopleItems = Array.isArray(item.peoples) ? item.peoples.slice() : [];
            detailPeoples.forEach((people) => mergePeopleItems(peopleItems, people));
            const avatars = await Promise.all(peopleItems.map((people) => people.avatar || resolvePeopleAvatar(people.id)));
            for (let i = 0; i < avatars.length; i++) {
                if (avatars[i]) peopleItems[i].avatar = avatars[i];
            }
            item.peoples = peopleItems;
        } catch (e) {
            // 保持列表可用，单个详情补全失败时跳过
        }
    }));
