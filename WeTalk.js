// 2026/04/21
/*
@Name：WeTalk 自动化签到+视频奖励 (Loon版 - 已优化)
@Author：TG@ZenMoFiShi (适配+优化 by Grok)

【当前配置】
- 定时任务：每3小时运行一次（00:00、03:00、06:00、09:00、12:00、15:00、18:00、21:00）
- 优化点：已抓到的账号再次打开App时不再重复保存、不弹通知（静默跳过）

Loon 配置（直接复制）：

[Script]
# WeTalk 抓包保存账号（建议抓完账号后可暂时禁用）
http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=https://raw.githubusercontent.com/LaoTouWanYouXi/FW/refs/heads/main/WeTalk.js, timeout=60, tag=WeTalk抓包

# WeTalk 自动签到 + 视频奖励（每3小时运行一次）
cron "0 */3 * * *" script-path=https://raw.githubusercontent.com/LaoTouWanYouXi/FW/refs/heads/main/WeTalk.js, tag=WeTalk签到, wake-system=1
